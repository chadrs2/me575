...
function wavedrag(r)
    ...
    finePointCount = Int64((LENGTH - 2 * dsTiny) / dxFine)
    tinyPointCount = Int64(2 * (dsTiny / dxTiny))
    totalPointCount = finePointCount + tinyPointCount + 1
    
    # Useful variables to be initialized only once
    X_LOCATIONS = zeros(15)
    X_MESH = zeros(totalPointCount)
    R_COARSE = zeros(15)
...
function Dfunc(x)
    return (x[2] - x[1])^2 + (x[3] - x[1])^2 + (x[4] - x[1])^2
end




function computeDrag(x, A, nmax)
    rs = 1
    ...
    for i = 1:nmax
      ...
      for j = 1:i 
        ...
        t1pm = -(c0 / 6) * (3 * g0 + 2 * g1 * (x1u - x2l) + 3 * g1 * x2l) * (x1u - x2l)^2
        t2pm = (c1 / 48) * (4 * g0 + 3 * g1 * (x1u - x2l) + 4 * g1 * x2l) * (x1u - x2l)^3
        t3pm = (-(c1 / 12) 
